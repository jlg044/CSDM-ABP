...
# Paths
dataset_path = 'c:/Users/esthe/Desktop/Entrenamiento/'
...
def load_sequence(sequence_path, img_shape):
    ...
    files = sorted(os.listdir(sequence_path))[:img_shape[0]]
    for i, file_name in enumerate(files):
        img_path = os.path.join(sequence_path, file_name)
        if os.path.exists(img_path):
            img = cv2.imread(img_path)
            img = cv2.resize(img, (img_shape[2], img_shape[1]))
...
train_ds, val_ds, num_classes = prepare_datasets(dataset_path, "train", IMG_SHAPE, batch_size=8, split_validation=True)
test_ds, _, _ = prepare_datasets(dataset_path, "test", IMG_SHAPE, batch_size=8, split_validation=False)
...
# ======================================================
def model_v1():
    sequence_input = Input(shape=IMG_SHAPE)

    cnn = TimeDistributed(Conv2D(32, (3,3), activation='relu'))(sequence_input)
    cnn = TimeDistributed(MaxPooling2D((2,2)))(cnn)
    cnn = TimeDistributed(BatchNormalization())(cnn)
    cnn = TimeDistributed(Dropout(0.4))(cnn)
...
# GUARDADO
# ======================================================
model.save('c:/Users/esthe/Desktop/modelo_acciones.keras')
